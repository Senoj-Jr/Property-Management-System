...
const VendorDashboard = () => {
  ...
  const handleAcceptIssue = async (issueId) => {
    try {
      const response = await axios.post(
        `http://localhost:8080/Vendor/${issueId}/accept/${vendor.vendor_id}`
      );
  ...
  const handleRejectIssue = async (issueId) => {
    try {
      const response = await axios.post(
        `http://localhost:8080/Vendor/${issueId}/reject/${vendor.vendor_id}`
      );
  ...
  const handleCompleteIssue = async (issueId) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/Vendor/${issueId}/complete/${vendor.vendor_id}`
      );
          ...
          >
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="bg-[#1A0000]/60 p-3 rounded-lg">
                ...
                <p className="text-[#D09683]">{vendor.serviceType}</p>
              </div>              
